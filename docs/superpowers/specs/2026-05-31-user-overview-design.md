...
- 用户可以通过一个 API Key 查看该 Key 本身的用量总览。
...
3. 当前密钥概览
   - 展示当前输入的 API Key
   - 展示 Key 名称、脱敏 Key、状态、今日消耗、今日请求数
   - 不展示同一用户下的其他 Key
4. 调用记录
   - 表格展示最近调用记录
   - 支持上一页、下一页、页码、每页数量
...
用户输入的 API Key 用于识别当前 Key。每次打开用户总览页时，服务端先同步一次 Sub2API 用户与 Key 列表，把最新 Key 信息写入本地数据库，再按 Key 哈希查找对应的 `api_key_id`。
...
总览和调用记录都按当前输入 Key 的 `api_key_id` 查询。
...
- 页面展示今日消耗、今日请求数、当前 Key、今日状态。
- 页面不展示 token 消耗。
- 页面只展示当前输入 Key 的名称、脱敏 Key、状态、今日消耗和请求次数。
- 调用记录支持分页切换。
- 前端不会出现管理员文案或实现细节文案。
- 所有自动化测试通过。
